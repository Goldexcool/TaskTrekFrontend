...
const Navbar = () => {
  ...
  return (
    <header className="border-b border-gray-800 sticky top-0 z-50 backdrop-blur-md bg-black/80">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <Link href="/" className="flex items-center space-x-2">
              <CheckSquare className="h-8 w-8 text-blue-400" />
              <span className="text-xl font-bold text-white">TaskTrek</span>
            </Link>
          </div>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="md:hidden py-4 overflow-hidden border-t border-gray-800 mt-4"
            >
              <div className="flex flex-col space-y-4">
                <Link 
                  href="#features" 
                  className="text-white hover:text-blue-400 transition-colors px-2 py-2"
                ...
                <div className="pt-2 flex flex-col space-y-3">
                  <Link 
                    href="/signIn" 
                    className="text-blue-400 font-medium hover:text-blue-300 transition-colors px-2 py-2"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    Sign in
                  </Link>
